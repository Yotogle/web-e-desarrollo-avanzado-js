    ...
    setTimeout(() => {
     ...
     // de lo contrario, recházala con un mensaje adecuado.
      if(mesasSolicitadas <= mesasDisponibles) {
        resolve(`Mesas disponibles: ${mesasDisponibles - mesasSolicitadas}. Reserva confirmada para ${mesasSolicitadas} personas.`);
      } else {
        reject(`Lo sentimos, no hay suficientes mesas disponibles. Solo quedan ${mesasDisponibles} mesas.`);
      }
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      if(Math.random() > 0.5) {
        ...
        reject(`Error al enviar el correo de confirmación a ${nombreCliente}. Inténtalo más tarde.`);
      }
